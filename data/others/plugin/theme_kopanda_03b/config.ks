...
;=========================================
	...
	tf.img_path_bgm   = tf.img_path + 'c_set_bgm.gif';
	tf.img_path_se    = tf.img_path + 'c_set_se.gif';
	tf.img_path_speed = tf.img_path + 'c_set_speed.gif';
	tf.img_path_auto  = tf.img_path + 'c_set_auto.gif';

	tf.img_path_bgmmute = tf.img_path + 'c_set_bgm0.gif';
	tf.img_path_semute  = tf.img_path + 'c_set_se0.gif';

	tf.img_path_skip_on  = tf.img_path + 'c_uts_on.png';
	tf.img_path_skip_off = tf.img_path + 'c_uts_off.png';

	// チェック画像の座標
	tf.config_x = [870, 280, 335, 390, 445, 500, 555, 610, 665, 720, 775];
	tf.config_y = [150, 220, 290, 360];
...
*load_bgm_img
	...
	[image layer="0" name="bgmvol" storage="&tf.img_path_bgmmute" x="&tf.config_x[tf.config_num_bgm]" y="&tf.config_y[0]"]
	[else]
	[image layer="0" name="bgmvol" storage="&tf.img_path_bgm" x="&tf.config_x[tf.config_num_bgm]" y="&tf.config_y[0]"]
	[endif]
[return]

*load_se_img
	[if exp="tf.config_num_se == 0"]
	[image layer="0" name="sevol"  storage="&tf.img_path_semute" x="&tf.config_x[tf.config_num_se]" y="&tf.config_y[1]"]
	[else]
	[image layer="0" name="sevol" storage="&tf.img_path_se" x="&tf.config_x[tf.config_num_se]" y="&tf.config_y[1]"]
	[endif]
[return]

*load_ch_img
	[image layer="0" name="ch" storage="&tf.img_path_speed" x="&tf.config_x[tf.config_num_ch + 1]" y="&tf.config_y[2]"]
[return]

*load_auto_img
	[image layer="0" name="auto"  storage="&tf.img_path_auto" x="&tf.config_x[tf.config_num_auto + 1]" y="&tf.config_y[3]"]
[return]

*load_skip_img
	[if exp="tf.text_skip == 'ON'"]
	[image layer="0" name="skip" storage="&tf.img_path_skip_on"  x="510" y="430"]
	[else]
	[image layer="0" name="skip" storage="&tf.img_path_skip_off" x="280" y="430"]
	[endif]
[return]
